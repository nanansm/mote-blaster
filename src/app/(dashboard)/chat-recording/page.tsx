...
import { Lock, MessageSquare, Plus, Trash2, ToggleLeft, ToggleRight, Edit2 } from 'lucide-react'
...
// ── Service account email untuk tutorial ──────────────────────────
const SERVICE_ACCOUNT_EMAIL = (() => {
  try {
    const creds = JSON.parse(process.env.NEXT_PUBLIC_GOOGLE_SA_EMAIL || '{}')
      ...
      <div className="rounded-xl bg-amber-50 border border-amber-200 p-5 text-sm text-slate-700 space-y-2">
        ...
        <ol className="list-decimal pl-5 space-y-1.5 leading-relaxed">
          ...
          <li>
            ...
            <div className="mt-1 bg-white rounded px-3 py-1.5 font-mono text-xs border border-amber-200 break-all select-all">
              {SERVICE_ACCOUNT_EMAIL ?? 'Lihat GOOGLE_SERVICE_ACCOUNT_JSON → client_email'}
            </div>
          </li>
          <li>
